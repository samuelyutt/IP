function training_slic_seg_label()
...
%% Extract superpixels
dir_path='../training/' % for testing or for training
srcFiles_img = dir([dir_path '*.jpg']);
srcFiles_labels = dir([dir_path '*.png']);
% may add some normalization to distance weight
filenum=length(srcFiles_img);% 2000 %50; %length(srcFiles_img); %50
all_Q = cell(1,filenum);
all_superpixel_labels = cell(filenum,1);

for a = 1:filenum
    filename = strcat(dir_path,srcFiles_img(a).name);
    im = imread(filename);
    im_lab = RGB2Lab(im);
    [L,N] = superpixels(im_lab,200,'IsInputLab',1);
    
    filename = strcat(dir_path,srcFiles_labels(a).name);
...
save('../train/all_Q.mat','all_Q');
save('../train/all_superpixel_labels.mat','all_superpixel_labels');
